...
let finaleBurstData = {
  ...
  lines: [
    ...
    { speaker: "Mew", name: "夢幻(幕後黑手)", text: "..." },
    { speaker: "Mew", name: "夢幻(幕後黑手)", text: "你以為這樣就結束了嗎？" },
    { speaker: "Mew", name: "夢幻(幕後黑手)", text: "見識一下真正的力量吧！" }
  ]
};
...
let preludeData = {
  ...
  lines: [
    ...
    { speaker: "Unknown", name: "???", text: "休想！！！" }, 
    { speaker: "Pikachu", name: "皮卡丘", text: "嗚哇！(冷錢包被打飛了)" },
    { speaker: "Mew", name: "夢幻(幕後黑手)", text: "嘻嘻嘻... 真是辛苦你了。" },
    { speaker: "Pikachu", name: "皮卡丘", text: "夢幻？ 為什麼是你？" },
    { speaker: "Mew", name: "夢幻(幕後黑手)", text: "那些資產本來就該屬於強者。既然你把助記詞找齊了，我就不客氣地收下了！" },
...
let finaleData = {
  ...
  lines: [
    { speaker: "Pikachu", name: "皮卡丘", text: "終於... 抓到你了！" },
    { speaker: "Mew", name: "夢幻(幕後黑手)", text: "嗚... 沒想到你竟然能看穿我的分身..." },
    { speaker: "Pikachu", name: "皮卡丘", text: "快把冷錢包的控制權還來！" },
    { speaker: "System", name: "系統", text: "(檢測到完整的助記詞... 正在恢復權限...)" },
    { speaker: "System", name: "系統", text: "權限恢復成功！資產已安全。" },
    { speaker: "Pikachu", name: "皮卡丘", text: "太好了！任務完成！" }
  ]
};
...
const wrongPhrases = ["行不行啊。", "你真的是他嗎。", "真失望。"];
...
function preload() {
  ...
  usaBg = loadImage('USAbg/USA.png');   // 伊布背景
  twBg = loadImage('TWbg/TW.png');      // 小火龍背景
  btcBg = loadImage('BTCbg/BTC.png');   // 傑尼龜背景
...
function drawFinalePrelude() {
  ...
  if (idx <= 2) {
    ...
    // 閃光效果
    if (frameCount % 6 < 3) background(255);
  ...
  if (isHovering) {
      ...
      if (mouseIsPressed) {
          ...
          if (mouseX >= sliderX - 10 && mouseX <= sliderX + sliderW + 10) {
             ...
             if (tfSound) tfSound.setVolume(volumeLevel);
...
function startFinaleQuiz() {
    ...
    // 從所有題庫中隨機選一題
    let allQuestions = [...twStockQuestions, ...cryptoQuestions, ...usStockQuestions];
